...
figure <- ggplot(plot_data) +
  geom_segment(
    ...
    aes(x = 0, xend = length, y = chromosome, yend = chromosome, color = gene_density), size = 0.7) + 
  geom_segment(
  ...
  scale_color_gradientn(
    colours = viridis(100),
    limits = c(min(scaffold_lengths$gene_density), max(scaffold_lengths$gene_density)),
    name = "Gene Density"
  ) +
  theme(
